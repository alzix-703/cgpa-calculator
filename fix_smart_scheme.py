...
<body class="bg-slate-900 text-slate-100 min-h-screen font-sans p-4 md:p-8">
        ...
        <div class="bg-slate-800/90 border border-slate-700/80 p-5 rounded-2xl shadow-xl space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                <div>
                    ...
                    <input type="number" id="userAge" placeholder="Enter age (e.g. 18)" class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-sm text-white focus:outline-none focus:border-amber-500">
                </div>
                <div>
    ...
    function findSchemes() {
        ...
        const age = rawAge ? parseInt(rawAge) : 0;
        const gender = document.getElementById('userGender').value;
        const category = document.getElementById('userCategory').value;
        const income = document.getElementById('userIncome').value;

        const filtered = schemesData.filter(function(s) {
            const ageMatch = !rawAge || (age >= s.minAge && age <= s.maxAge);
        ...
        for (let i = 0; i < filtered.length; i++) {
            const s = filtered[i];
            html += '<div class="bg-slate-800/80 border border-slate-700/80 hover:border-amber-500/40 p-4 rounded-xl space-y-3 flex flex-col justify-between shadow-md">' +
                '<div class="space-y-2">' +
