...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, sans-serif; }
        ...
        .home-link { display: inline-block; margin-top: 15px; color: #60a5fa; font-size: 0.85rem; text-decoration: none; }
    </style>
</head>
<body>
    <div class="card">
        ...
        <a href="/" class="home-link">🏠 Return to Main Utility Portal</a>
    </div>
    ...
    <script>
        ...
        // Dynamic local memory storage so generated 5-digit codes stay saved
        const database = {
            "shayari": {
        ...
        function loadContent() {
            ...
            let randomCode = String(Math.floor(10000 + Math.random() * 90000));
            ...
            if(currentMode === 'shayari') {
                ...
                generatedHtml = `<span class="code-tag">Saved Code: ${randomCode}</span><br>🔥 <b>7 Unseen Shayaris (${val}):</b><br><br>` + shayarisList.join('<br><br>');
            } else {
                generatedHtml = `<span class="code-tag">Saved Code: ${randomCode}</span><br>📖 <b>${val} (High Dopamine Story):</b><br><br>Rohan hamesha library ke corner table par baithta tha, sirf Ananya ko dekhne ke liye. Ananya jab bhi wahan se guzarti, Rohan ka heart rate double ho jaata tha. Ek din Rohan ne apna poora sach ek letter mein likha... Par shayad kismat ko kuch aur manzoor tha. Ananya ki shaadi kisi aur se fixed ho chuki thi. Rohan ne woh letter kabhi Ananya ko nahi diya, balki apni kitaab ke beech chhupa diya. Aaj bhi jab woh kitaab khulti hai, toh wahi silent sacrifice ki mehak aati hai...`;
            }

            // Save in database array in real-time
            database[currentMode][randomCode] = generatedHtml;
        ...
        function fetchByCode() {
            ...
            if(code.length !== 5 || isNaN(code)) {
                ...
                err.innerText = "❌ Invalid Code! Code must be 5 digits or enter secret key.";
                return;
            }
